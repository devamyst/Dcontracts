...
import me.devamy.contracts.api.events.PlayerCancelOrderEvent;
import me.devamy.contracts.api.events.PlayerCollectItemsEvent;
import me.devamy.contracts.api.events.PlayerCreateOrderEvent;
import me.devamy.contracts.api.events.PlayerDeliverOrderEvent;
import me.devamy.contracts.gui.YourOrderGUI;
...
import static me.devamy.contracts.utils.ConvertUtils.formatNumber;

// TODO: Replace `item` with OrderItem instead of ItemStack.
// Problem: how to store it in database?
public class Order implements me.devamy.contracts.api.Order {
    public final int id;
    ...
    public Response collect(int amount) {
        ...
        if (amount > config.maxCollect && !p.hasPermission("contracts.bypass.max-collect")) {
            p.sendRichMessage(config.exceedMaxCollect);
            return Response.FAIL;
        }

        final int collectedInMinute = PDCUtils.getCollected(p);
        if (collectedInMinute > config.maxCollectPerMinute && !p.hasPermission("contracts.bypass.max-collect-per-minute")) {
