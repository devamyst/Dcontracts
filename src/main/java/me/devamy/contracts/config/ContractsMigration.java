...
public class ContractsMigration {
    ...
    public static void perform(final @NotNull Config config) throws Exception {
        ...
        if (configVersion == Config.CURRENT_CONFIG_VERSION) {
            ...
            config.yourContractsGUIConfig.saveToFile();
            ...
            config.newContractDialogConfig.saveToFile();
            config.confirmDeliveryDialogConfig.saveToFile();
            config.manageContractDialogConfig.saveToFile();
        ...
        config.yourContractsGUIConfig.migrateV5(config.configFile);
        ...
        config.newContractDialogConfig.migrateV5(config.configFile);
        config.confirmDeliveryDialogConfig.migrateV5(config.configFile);
        config.manageContractDialogConfig.migrateV5(config.configFile);
        ...
        if (prefix == null) {
            Log.warn("Missing 'sort-prefix' in config during migration, using default");
        }
        for (final SortType sort : SortType.values()) {
            final String display = config.configFile.getString("sort-types." + sort.getIdentifier());
            if (display == null) {
                Log.warn("Missing sort display for '" + sort.getIdentifier() + "' during migration, skipping");
                continue;
            }
            sort.setDisplayActive(prefix == null ? display : prefix + display);
    ...
    public static void setDefaultV4(final @NotNull ConfigFile config) {
        ...
        config.addDefault("expires-after", 7L * 24L * 60L * 60L * 1000L, "After this amount of millisecond(s), the contract will be expired");
        config.addDefault("minimum-price", 0.1, "The minimum amount of money per item the player can create a contract with");
        ...
        config.addDefault("max-collect-per-minute", 1000,
                """
                        ...
                        Setting this too high might allow players to lag the server with large contracts
...
//                "minecraft:custom_data", // Custom data doesn't exist in the registry for some reason?
                ...
                This similarity check happens when a player delivers a contract, it accepts items in the delivery inventory that are similar to the one in the contract
        ...
        config.addDefault("shulker-delivering", true, "Whether to allow players to deliver contracts with items in shulker boxes");
        config.addDefault("broadcast-contract-creation", false, "Whether to broadcast a server-wide message when a contract is created");

        // MESSAGES
        config.addDefault("messages.create-contract-success", "<gray>Your contract has been created");
        config.addDefault("messages.invalid-input", "<red>Invalid number or format");
        config.addDefault("messages.deliver", "<gray>You earned <green>$<money><gray> from delivering a contract");
        ...
        config.addDefault("messages.deliver-self", "<red>You cannot deliver your own contract");
        ...
        // CONTRACT STATUS
        config.addComment("order-status", "How should different types of contract status appear\nThey will be used with <contract-status> placeholder");
        config.addDefault("order-status." + OrderStatus.AVAILABLE.getIdentifier(), "<gray>Expires after <day>d <hour>h <minute>m <second>s");
        config.addDefault("order-status." + OrderStatus.EXPIRED.getIdentifier(), "<red>Contract Expired");
        config.addDefault("order-status." + OrderStatus.COMPLETED.getIdentifier(), "<green>Contract Completed");

        // MAIN GUI
        config.addDefault("gui.main.title", "Contracts");
        config.addDefault("gui.main.order-lore", List.of(
                ...
                "",
                "<white>Click to deliver <aqua><player><white>'s contract"
        ));
        ...
        new SlotInfo(6, List.of("<white>Click to view your contracts"), "<aqua>Your Contracts", ItemType.CHEST).addDefault(config, "gui.main.buttons.your-orders");
        ...
        // YOUR CONTRACTS GUI
        config.addDefault("gui.your-orders.title", "Your Contracts");
        ...
        new SlotInfo(-1, List.of("<white>Click to create a new contract"), "<aqua>New Contract", ItemType.MAP).addDefault(config, "gui.your-orders.buttons.new-order");
        ...
        // NEW CONTRACT DIALOG
        config.addDefault("gui.new-order.title", "Create A New Contract");
        config.addDefault("gui.new-order.item-description", "You're creating a contract for this item");
        ...
        config.addDefault("gui.new-order.confirm-tooltip", "Click to confirm the contract");
        ...
        config.addDefault("gui.manage-order.title", "Manage Contract");
        config.addDefault("gui.manage-order.body", "You are managing this contract");
        config.addDefault("gui.manage-order.collect-items-button", "Collect Items");
        config.addDefault("gui.manage-order.collect-items-tooltip", "Click to collect items from this contract");
        config.addDefault("gui.manage-order.cancel-order-button", "Cancel Contract");
        config.addDefault("gui.manage-order.cancel-order-tooltip", "Click to cancel the contract");

        // COLLECT ITEMS DIALOG
        config.addDefault("gui.collect-items.title", "Collect Items");
        config.addDefault("gui.collect-items.body", "You are collecting items from this contract. You can collect up to <aqua><in-storage> <item>");
        ...
        config.addDefault("gui.cancel-order.title", "Cancel Contract");
        config.addDefault("gui.cancel-order.body", "You are cancelling this contract. It will be expired");
        config.addDefault("gui.cancel-order.cancel-button", "<red>Cancel");
        config.addDefault("gui.cancel-order.cancel-tooltip", "Click to cancel the cancellation of this contract");
        config.addDefault("gui.cancel-order.confirm-button", "<green>Confirm");
        config.addDefault("gui.cancel-order.confirm-tooltip", "Click to confirm the cancellation of this contract");
